...
import os
from scripts.evaluation_bb import save_videos
...
from evaluation.adept.eval_adept import eval_adept
...
def evaluate(cfg: Configuration, dataset: Dataset, file, n, plot_frequency= 1, plot_first_samples = 2):
    ...
    # Plotting
    plot_attention = True
    demo_mode = True # True:plot two samples of the test set  False: run evaluartion without plots on all samples
    plot_first_samples = 2 if demo_mode else 0

    # settings to play around with
    cfg_net = cfg.model
    cfg_net.inner_loop_enabled = True       # *whether to use the percept gate* default: True
    cfg_net.num_objects = 4                 # *number of slots* default 4
    cfg_net['visiblity_baseline'] = False   # *control the percept gate with the slot's occlusion factor* default False: only works if inner_loop_enabled = True
    # SETTINGS END ---------
    ...
    set_test_array, evaluation_modes = get_evaluation_sets(dataset, demo_mode)
        ...
        for evaluation_mode in evaluation_modes:
            ...
            with th.no_grad():
                for i, input in enumerate(dataloader):
                    ...
                    # log video
                    if (i < plot_first_samples):
                        video_tensor = th.stack(video_list, dim=0)[None]
                        save_videos(video_tensor, f'{plot_path}/object{i}', verbose=verbose)
...
def get_evaluation_sets(dataset, demo_mode=False):
    ...
    if demo_mode:
        set_control['samples'] = set_control['samples'][1:3]
        set_surprise['samples'] = set_surprise['samples'][2:4]
    set_test_array = [set_surprise]
