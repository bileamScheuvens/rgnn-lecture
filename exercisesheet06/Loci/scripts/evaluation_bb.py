...
def evaluate(cfg: Configuration, dataset: Dataset, file, n, plot_first_samples = 0):
    ...
    cfg_net.num_objects = 6                 # *number of slots* default 6
    ...
    cfg.defaults.teacher_forcing = 14      # *number of cycles the first image is fed into Loci as warmup*  default 14 (2)
    # SETTINGS END ---------
        ...
        for evaluation_mode in evaluation_modes:
            ...
            with th.no_grad():
                for i, input in enumerate(dataloader):
                    ...
                    gt_pos_batch          = th.zeros((cfg_net.batch_size, rollout_length, 6, 2)).to(device)
                    ...
                    # sanity check
                    if (num_rollout != rollout_length) and (num_burnin != burn_in_length) and (evaluation_mode == 'vidpred'):
...
def save_videos(video_tensor, plot_path, verbose=False, fps=10):
    video_tensor = video_tensor.cpu().numpy()
    img_path = plot_path + '/img'
    print("SAVING VIDEO.... in evalution_bb.py line 291", img_path)
...
def get_evaluation_sets(dataset):
    ...
    evaluation_modes = ['open', 'vidpred_black', 'vidpred_auto'] # use 'open' for no blackouts
    set_test_array = [set]
