...
def test_linear_function():
    ...
    input = torch.rand(batch_size, input_features, dtype=torch.double, requires_grad=True)
    weight = torch.rand(output_features, input_features, dtype=torch.double, requires_grad=True)
    bias = torch.rand(output_features, dtype=torch.double, requires_grad=True)
...
def test_relu_function():
    ...
    batch_size = 2
    input = torch.rand(batch_size, dtype=torch.double, requires_grad=True)
...
def test_relu_function_near_zero():
    ...
    batch_size = 2
    input = 0.1 * (torch.rand(batch_size, dtype=torch.double, requires_grad=True)-0.5)
