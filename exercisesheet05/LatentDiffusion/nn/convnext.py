...
class FiLMLayer(nn.Module):
    def __init__(self, num_hidden, num_conditioning, eps = 1e-5):
        super(FiLMLayer, self).__init__()
        self.eps = eps
        self.weight = nn.Parameter(th.ones(1, num_hidden, 1, 1))
        self.bias   = nn.Parameter(th.zeros(1, num_hidden, 1, 1))

        self.weight_alpha = nn.Parameter(th.zeros(1) + 1e-6)
        self.bias_alpha   = nn.Parameter(th.zeros(1) + 1e-6)

        self.mlp = nn.Sequential(
            nn.Linear(num_conditioning, max(num_conditioning, num_hidden) * 4),
            nn.SiLU(),
            nn.Linear(max(num_conditioning, num_hidden) * 4, num_hidden*2)
        )

    def forward(self, input, conditioning):
        mean = th.mean(input, dim=1, keepdim=True)
        var  = th.var(input, dim=1, keepdim=True)

        weight, bias = self.mlp(conditioning).unsqueeze(-1).unsqueeze(-1).chunk(2, dim=1)

        weight = self.weight + self.weight_alpha * weight
        bias   = self.bias   + self.bias_alpha   * bias

        return ((input - mean) / th.sqrt(var + self.eps)) * weight + bias
...
class FiLMConvNeXtBlock(nn.Module):
    def __init__(
        ...
        self.conv = nn.Conv2d(in_channels, in_channels, kernel_size=7, padding=3, groups=in_channels)
        self.film = FiLMLayer(in_channels, conditioning_channels)
        self.mlp  = MemoryEfficientBottleneck(in_channels, out_channels)

        self.skip  = SkipConnection(in_channels, out_channels)

        self.alpha = nn.Parameter(th.zeros(1) + 0.1)

    def forward(self, input: th.Tensor, conditioning: th.Tensor) -> th.Tensor:
        return self.alpha * self.mlp(self.film(self.conv(input), conditioning)) + self.skip(input), conditioning

class PatchUpscale(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size = 4):
        super(PatchUpscale, self).__init__()
        assert in_channels % out_channels == 0 or out_channels % in_channels == 0, f"Number of input channels ({in_channels}) must be a multiple of the number of output channels ({out_channels})"
    ...
    def forward(self, input, conditioning=None):
        if conditioning is not None:
            return self.skip(input), conditioning
...
class PatchDownscale(nn.Module):
    ...
    def forward(self, input: th.Tensor, conditioning=None) -> th.Tensor:
        H, W = input.shape[2:]
        K    = self.kernel_size
        ...
        if conditioning is not None:
            return skip + residual, conditioning
