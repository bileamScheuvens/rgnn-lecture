...
def update_path(cfg, scratch_path):
    source_path  = cfg['data']['data_dir']
    updated_path = scratch_path
    print(f"Updating: {source_path} -> {updated_path}")
    cfg['data']['data_dir'] = updated_path
...
if __name__ == "__main__":
    ...
    parser.add_argument("-load-vae", "--load-vae", default="", type=str)
    ...
    mode_group.add_argument("-train-denoiser", "--train-denoiser", action="store_true")
    mode_group.add_argument("-train-vae", "--train-vae", action="store_true")
    ...
    if args.train_denoiser or args.validate:
        training.train_denoiser(cfg, args.load_vae, args.load if args.load != "" else None)
    if args.train_vae:
        training.train_vae(cfg, args.load if args.load != "" else None)
